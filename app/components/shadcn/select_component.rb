...
class Shadcn::SelectComponent
  ...
  def call
    view_context.content_tag :select, @content, name:,
                                                class: tw('rounded-md border bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50', @options[:class]) # rubocop:disable Layout/LineLength
  end
end
