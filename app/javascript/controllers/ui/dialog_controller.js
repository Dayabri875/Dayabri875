...
export default class UIDialog extends Controller {
  ...
  openBy(target) {
    const poppoverTarget = this.dialogTarget.closest('[data-controller="ui--transition"]')

    if(poppoverTarget) {
      poppoverTarget.style.removeProperty('transform')
      poppoverTarget.style.removeProperty('margin')
      poppoverTarget.style.top = '999999px'
      poppoverTarget.dataset.state = "closed";
