module Components::ButtonHelper
  def render_button(label = "", text: nil, variant: :default, as: :button, href: nil, data: {}, name: nil, form: nil, value: nil, id: nil, target: nil, **options, &block)
    ...
    when :cancel
      " bg-transparent border border-[#D90D0D] text-[#D90D0D] hover:bg-[#D90D0D]/10"
    when :back
      " bg-gray-300 hover:bg-gray-200"
    when :success
      " border-success-foreground bg-success-foreground text-success"
    end
    button_classes = tw(button_classes, variant_classes, options[:class])

    text = label if label.present?
    text = capture(&block) if block
    render "components/ui/button", text:, button_classes:, as:, href:, data:, name:, value:, form:, id:, target:, **options
  end
end
